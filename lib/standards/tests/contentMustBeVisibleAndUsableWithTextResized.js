  ...
  test: function ({ $, fail, warn, pageConfiguration, ask }) {
    ...
    if (typeof require === 'function') {
      window.top.launchTextResizedWindow = function () {
        ...
        textResizedWindow = new BrowserWindow({
        ...
        require('@electron/remote/main').enable(textResizedWindow.webContents);
        textResizedWindow.loadURL(pageConfiguration.url);
        textResizedWindow.webContents.executeJavaScript(
          'window.document.body.style.fontSize = "200%"'
        );
      };
