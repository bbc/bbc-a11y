...
function A11yWorld() {
  ...
  this.runA11yInteractively = (args) => {
    return new Promise((resolve, reject) => {
      ...
      const interactiveProcess = spawn(this.binaryPath, splitArgs, {
        cwd: this.tempDir,
      });
      setTimeout(() => {
        interactiveProcess.kill('SIGINT');
      }, 2000);
  ...
  this.answerManualTestQuestions = (questionsAndAnswers) => {
    const answerNextQuestion = () => {
      ...
      return this.answerFrameMonkey
        .find('h1', { text: qa.question })
        .shouldExist()
  ...
  this.answerAllManualTestQuestionsWithOneFail = () => {
    const answerRestAsPass = () =>
      this.answerFrameMonkey
        .find('.pass-button')
        .click({ timeout: 50 })
...
After(function (scenario) {
  if (scenario.result.status === 'passed' && this.pageFrame)
    this.pageFrame.style.display = 'none';
});
