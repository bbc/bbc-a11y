 ...
 PrettyReporter.prototype.summariseTests = function (standards) {
   this.commandLineConsole.log('Tests covered by bbc-a11y:\n')
   const tests = Object.values(standards.sections).reduce((tests, section) => {
     return tests.concat(section.tests)
   }, [])
   this.commandLineConsole.log(tests.map(test => [
     `Section: ${test.section.title}`,
     `Name: ${test.name}`,
     `Type: ${test.type}`,
     [].concat(test.failsForEach).map(f => `Fail: for each ${f}`).join('\n')
   ].join('\n')).join('\n\n'))
 }
