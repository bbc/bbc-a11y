...
function askInFrame (question) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var notesArea = jquery('<textarea />').appendTo(answerFrame)
    var passButton = jquery('<button>Pass</button>').appendTo(answerFrame)
    passButton.on('click', function () {
      resolve()
    })
    var failButton = jquery('<button>Fail</button>').appendTo(answerFrame)
...
Runner.prototype.run = function (pages, windowAdapter, reporter, exit) {
  ...
  return this.configForPages(pages)
    ...
    .then(function (config) {
      return run(config, windowAdapter, reporter, ask, exit)
    })
}
