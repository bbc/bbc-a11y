...
PrettyReporter.prototype.pageChecked = function (page, pageResult) {
  ...
  const hasErrors = pageResult.results.some(r => r.errors.length > 0)
  let statusLine = (hasErrors ? '✗' : '✓') + ' ' + page.url
  ...
  for (let i = 0; i < pageResult.results.length; i++) {
    const result = pageResult.results[i]
...
PrettyReporter.prototype.runEnded = function () {
  if (this.summary.documentationUrls.length > 0) {
    ...
    for (let i = 0; i < this.summary.documentationUrls.length; i++) {
